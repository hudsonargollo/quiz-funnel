...
## 2026-08-17 — New React admin app: dashboard, mobile-redesigned builder, Offer Finder, account recovery

The biggest single day of shipped work on the platform to date, spanning the new `admin-src/` React rewrite (in progress since 2026-08-15, committed to git for the first time today), the legacy admin, and the CRM/AI Ads backend.

- **New dashboard + funnel settings drawer** (`admin-src/`, replaces the placeholder stub at `/app`): funnel switcher rail, stat tiles, conversion funnel, and 14-day leads chart, all wired to the real `/api/funnels` and `/api/crm/stats` endpoints. The funnel settings panel — previously one flat scroll mixing naming, publish state, two redundant "edit" buttons, plaintext Stripe keys, and raw JSON config — is now grouped into Overview/Payments/Advanced tabs, with publish state as a first-class segmented control (not a buried dropdown) and Stripe secret/webhook keys correctly treated as write-only (the API never returns them, so the old "reveal" toggle was pointless).
- **Funnel builder redesigned for mobile**: the 3-pane desktop layout doesn't fit a phone, so mobile gets its own model instead of a cramped squeeze — full-bleed live preview as the primary view, screen list and property inspector moved into bottom sheets (`vaul`) reached via a bottom tab bar, and the header minimizes into a hamburger menu for secondary actions. Desktop keeps the identical 3-pane layout. Added `framer-motion` for an animated shared-element highlight between selected screens, a cross-fade on screen switch, and an animated save-state.
- **Forgot/reset password flow**, using the existing Resend integration (no new provider). Tokens are single-use, expire in 60 minutes, enumeration-safe (`{ok:true}` regardless of whether the account exists), and a completed reset invalidates every existing session for that user.
- **Fixed a real bug**: the login screen was swallowing the server's actual error message ("Invalid email or password") on every failed attempt, showing nothing instead — a 401-handling helper meant for "session expired mid-use" was wrongly applied to the login call itself.
- **Offer Finder** (first git commit — the feature and its migration existed only on disk/prod since 2026-08-15): lets affiliate accounts pull products from Hotmart (own connected app credentials) or ClickBank (paste-a-link OG-tag unfurl) and convert one into a funnel, mirroring the existing swipe-file mining flow.
- **Meta Ad Library search never actually showed images** — not a bug exactly, a design gap: Meta's public API has no image field, only a link to an interactive snapshot *page*, and the UI correctly never tried to render that as `<img>`. Fixed by fetching each snapshot page server-side and extracting its `og:image` (same technique already used for ClickBank link unfurling), with a graceful fallback to the old link-only view when extraction fails. Separately noted: `META_ADS_TOKEN` isn't currently configured in production, so Meta search returns zero results regardless until that secret is set.
- **AI-generated ad creative images** get a retry path: the image prompt is now persisted per creative (it was only ever held in memory for the original call), so a failed or skipped generation can be retried from a button on the card without regenerating the copy or spending extra credits. Root-caused the actual production failures while testing this: OpenAI's account backing `OPENAI_API_KEY` is out of billing credits — not a code or config problem.
- AI Ads generation (strategy, competitor analysis, ad copy, quiz-copy, scoring) was silently defaulting every response to English regardless of the admin's PT/EN toggle — now passes the current language through.
